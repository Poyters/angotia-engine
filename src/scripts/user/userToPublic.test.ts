import { userToPublic } from "./userToPublic";
...
describe("userToPublic test suite", () => {
  ...
  const publicExampleUser: any = userToPublic(exampleUser);
  ...
  const publicInvalidId: any = userToPublic(invalidId);

  test("is a function", () => {
    expect(typeof userToPublic).toEqual("function");
  });
